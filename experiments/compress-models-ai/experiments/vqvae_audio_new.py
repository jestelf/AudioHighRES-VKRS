...
Изменения:
  ...
  - Добавлен режим тестирования, который загружает сохранённую модель и запускает её на новых данных (LibriSpeech test-clean).
"""
...
def train_main():
    ...
    # Сохраняем модель
    model_path = os.path.join(backup_dir, "model.pth")
    torch.save(model.state_dict(), model_path)
    logging.info(f"Model saved to {model_path}")

# ------------- Функция тестирования (test mode) -------------
def test_main(model_path):
    ...
    # Используем те же архитектурные параметры, что и при обучении
    training_params = {
        "in_channels": 1,
        "hidden_channels": 256,
        "latent_dim": 64,
    ...
    # Загружаем тестовый датасет (например, LibriSpeech test-clean)
    dataset = torchaudio.datasets.LIBRISPEECH(root="data", url="test-clean", download=True)
    # Можно взять подмножество для быстроты теста
    ...
    logging.info("Testing model on new data (LibriSpeech test-clean)...")
    test_autoencoder(model, dataloader, device, output_dir="output_test")

# ------------- Основной блок (выбор режима: train или test) -------------
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Audio Autoencoder Training and Testing")
    parser.add_argument("--mode", choices=["train", "test"], default="train",
                        help="Режим работы: 'train' для обучения, 'test' для запуска сохранённой модели на новых данных")
    parser.add_argument("--model_path", type=str,
                        help="Путь к сохранённой модели (обязательный параметр для режима test)")
    ...
    if args.mode == "train":
        ...
        if not args.model_path:
            raise ValueError("Для режима test необходимо указать --model_path (например, --model_path path/to/model.pth)")
