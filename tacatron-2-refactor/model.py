...
class Encoder(nn.Module):
    # без изменений, как было. Для компактности слегка сокращён
    def __init__(self, hparams):
...
class Tacotron2(nn.Module):
    ...
    def parse_batch(self, batch):
        """
        batch: tuple containing (text_padded, input_lengths, mel_padded, gate_padded, output_lengths, spk_ids, emo_ids)
        Returns:
            x: tuple containing (text_padded, input_lengths, mel_padded, gate_padded, output_lengths, spk_ids, emo_ids)
            y: tuple containing (mel_padded, gate_padded)
        """
    ...
    def forward(self, inputs):
        """
        inputs: tuple containing (text_padded, input_lengths, mel_padded, gate_padded, output_lengths, spk_ids, emo_ids)
        Returns:
            mel_outputs, mel_outputs_postnet, gate_outputs, alignments
        """
        text_padded, input_lengths, mel_padded, gate_padded, output_lengths, spk_ids, emo_ids = inputs

        embedded_text = self.embedding(text_padded).transpose(1, 2)
        encoder_outputs = self.encoder(embedded_text, input_lengths)

        # Add speaker and emotion embeddings
        spk_e = self.spk_embed(spk_ids)  # [B, spk_emb_dim]
        emo_e = self.emo_embed(emo_ids)  # [B, emo_emb_dim]
        style_cat = torch.cat([spk_e, emo_e], dim=1)  # [B, spk_emb_dim + emo_emb_dim]

        # Expand style embeddings to match encoder time steps
        B, T_enc, E_enc = encoder_outputs.shape
        sty = style_cat.unsqueeze(1).expand(B, T_enc, style_cat.size(1))
        encoder_outputs = torch.cat([encoder_outputs, sty], dim=2)  # [B, T_enc, E_enc + spk_emb + emo_emb]
    ...
    def parse_output(self, outputs, output_lengths=None):
        """
        Apply mask to the outputs if necessary
        """
    ...
    def inference(self, inputs, spk_id, emo_id):
        """
        inputs: text sequence [B, seq_len]
        spk_id: [B]
        emo_id: [B]
        Returns:
        ...
        B, T_enc, E_enc = encoder_outputs.shape
        spk_e = self.spk_embed(spk_id)
        emo_e = self.emo_embed(emo_id)
        style_cat = torch.cat([spk_e, emo_e], dim=1).unsqueeze(1).expand(B, T_enc, style_cat.size(1))
